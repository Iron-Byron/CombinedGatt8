...
int main(void)
{
    ...
    nrf_gpio_pin_clear(pwr_pins[0]);
    nrf_gpio_pin_clear(pwr_pins[1]);
    nrf_gpio_pin_clear(pwr_pins[2]);
    nrf_gpio_pin_clear(pwr_pins[3]);
    nrf_gpio_pin_clear(pwr_pins[4]);
    nrf_gpio_pin_clear(pwr_pins[5]);
            ...
            if(loopCnt<=5)
            {
                if (loopCnt <= 5) 
                {
                    ...
                    if (arrayIndex >= 1 && arrayIndex <= 5) 
                    {
                        ...
                        // printk("Switching PwrPin: %d on\n", arrayIndex); //Moved into the takereading if statement
